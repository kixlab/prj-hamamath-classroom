...
export const SubQs = () => {
  ...
  useEffect(() => {
    ...
    saveResult(currentProblemId, undefined, undefined, currentSubQuestionData ?? undefined, preferredVersion, undefined, userId);
  }, [currentProblemId, currentSubQuestionData, preferredVersion, userId]);
  ...
  const [previewOpen, setPreviewOpen] = useState<boolean>(false); // 학습지 미리보기 모달
  const bModeSyncedProblemId = useRef<string | null>(null);
  const subqAutoStartConsumedRef = useRef(false);
  ...
  // 저장된 subQuestion만 복원된 경우(중간 이탈 후 재진입) — 진행 중 생성 UI 복원
  useEffect(() => {
    if (!currentProblemId) {
    ...
    const generatedCount = (((currentSubQuestionData as any)?.guide_sub_questions ?? []) as SubQuestion[]).length;
    const isNewProblem = bModeSyncedProblemId.current !== currentProblemId;

    if (finalizedSubQuestionForRubric != null) {
      setBMode(false);
      ...
      const area = (currentSubQuestionData as any)?.subject_area;
  ...
  }, [currentProblemId, currentCotData, currentSubQuestionData, finalizedSubQuestionForRubric]);

  const containerRef = useMathJax([
    (currentSubQuestionData as any)?.guide_sub_questions,
    preferredVersion,
    locale,
    verificationStates,
  ...
  const handleFinalize = () => {
    ...
    if (!confirmed) return;
    try {
      const gd = currentSubQuestionData as any;
      if (gd?.guide_sub_questions?.length) {
        ...
        setFinalizedSubQuestionForRubric(finalizedPayload);
        // 확정 시 서버·사이드바에 저장 (다른 기기/새로고침 시에도 목록에 표시)
        const problemId = finalizedPayload.problem_id ?? currentProblemId;
        if (problemId) {
          const subQuestionToSave = { ...gd, guide_sub_questions: finalizedSubs };
          saveResult(problemId, undefined, undefined, subQuestionToSave, preferredVersion, undefined, userId);
        }
  ...
  const generateNextStepB = async () => {
    ...
    const guideSubQuestions: SubQuestion[] =
      ((currentSubQuestionData as any)?.guide_sub_questions as SubQuestion[]) || [];
    ...
    try {
      ...
      // 1단계: 현재 단계의 원본문항 생성
      const subQuestionResponse = await api.generateSingleSubQuestion({
        main_problem: (currentCotData as any).problem,
        main_answer: (currentCotData as any).answer,
        main_solution: (currentCotData as any).main_solution || null,
        grade: (currentCotData as any).grade,
      ...
      (setCurrentSubQuestionData as any)(subQuestionPayload);
  ...
  const startSubQuestionGeneration = () => {
    setFinalizedSubQuestionForRubric(null);
    setBMode(true);
    setBVisibleCount(0);
    setBCurrentIndex(0);
  ...
  useEffect(() => {
    if (!pendingSubqAutoStart || currentStep !== 3) return;
    if (!currentCotData || !(currentCotData as any).steps) return;
    if (isGeneratingSteps || subqAutoStartConsumedRef.current) return;

    const guideSubQuestions: SubQuestion[] =
      ((currentSubQuestionData as any)?.guide_sub_questions as SubQuestion[]) || [];
    const totalSteps = (currentCotData as any).steps.length;
    if (guideSubQuestions.length >= totalSteps) {
      setPendingSubqAutoStart(false);
      return;
    }

    subqAutoStartConsumedRef.current = true;
    setPendingSubqAutoStart(false);
    startSubQuestionGeneration();
  }, [
    pendingSubqAutoStart,
    currentStep,
    currentCotData,
  ...
  const toggleVerification = (subqId: string, version: SubqPanelVersion) => {
    ...
    if (next && (currentSubQuestionData as any)?.guide_sub_questions) {
      const subQ = (currentSubQuestionData as any).guide_sub_questions.find((q: SubQuestion) => q.sub_question_id === subqId);
      if (subQ) {
  ...
  const handleFeedbackRegenerate = async (
    ...
    if (isDemoMode) {
      try {
        ...
        if (version === "regenerated") {
          setEditingRegeneratedStates((prev) => ({ ...prev, [subqId]: false }));
        } else {
          setEditingOriginalStates((prev) => ({ ...prev, [subqId]: false }));
        }
      } finally {
        setRegeneratingStates((prev) => ({
          ...prev,
    ...
    try {
      const regenerateResponse = await api.regenerateSingleSubQuestion({
        ...
        subject_area: currentSubQuestionData.subject_area,
        considerations: (currentCotData as any).considerations || [],
        ...
        {!loading && !error && !isGeneratingSteps && (
          <div className={styles.emptyState}>
            ...
            <button
              ...
              onClick={startSubQuestionGeneration}
            >
              {t('subq.generateSubq')}
            </button>
          </div>
  ...
  const allSubQuestions = ((currentSubQuestionData as any).guide_sub_questions || []) as SubQuestion[];
  ...
  const incompleteGeneration =
    finalizedSubQuestionForRubric == null && generatedCount > 0 && generatedCount < totalCotSteps;
  ...
  const showBModeActions =
    finalizedSubQuestionForRubric == null &&
    inProgressiveMode &&
    !restAutoGenerated &&
    nextStepIndex < totalCotSteps &&
    visibleSubQuestions.length > 0;
  const showFinalizePrompt =
    finalizedSubQuestionForRubric == null && !showBModeActions && allSubQuestions.length > 0;
  const showPostFinalizeActions = finalizedSubQuestionForRubric != null;
  ...
  return (
    <div className={styles.subQuestionContainer} ref={containerRef}>
      {loading && (
        <div className={styles.loading}>
          <div className={styles.spinner}></div>
          <div>{t('common.loading')}</div>
      ...
      {(showBModeActions || showFinalizePrompt || showPostFinalizeActions) && (
