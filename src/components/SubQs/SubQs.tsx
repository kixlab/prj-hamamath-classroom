...
export const SubQs = () => {
  ...
  const generateNextStepB = async () => {
    ...
    setError(null);
    // 전체 로딩 오버레이 미사용 → 원본 반영 직후 화면에 표시, 재생성 영역은 "준비중"으로 표시
  ...
  const generateRemainingStepsB = async () => {
    ...
    setRestAutoGenerated(true); // 이후 문항 박스에서는 확정/나머지 버튼 숨김
    setError(null);
    // 전체 로딩 오버레이를 쓰지 않음 → 원본 반영 직후 화면에 표시되고, 재생성 영역만 "준비중"으로 표시
  ...
  return (
    <div className={styles.guidelineContainer} ref={containerRef}>
      {/* 문항 생성 중(하위문항 생성하기 / 확정 후 다음 문항 / 나머지 자동 생성)에는 항상 로딩 표시 */}
      {loading && (
        <div className={styles.loading}>
          <div className={styles.spinner}></div>
          <div>로딩 중...</div>
